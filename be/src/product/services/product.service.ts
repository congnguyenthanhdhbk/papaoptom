...
export class ProductService {
    ...
    constructor(
        @InjectModel("NotFoundProduct") private readonly notFoundModel: Model<NotFoundProduct>,
        @InjectModel("ForsageProduct") private readonly productModel: Model<Product>,
        ...
        fs.readFile(fileName, async (error, data) => {
            ...
            if (error) {
                ...
                try {
                    ...
                    const productIds = await Promise.all(ids.map((id) => ({
                        sku: id.id,
    ...
    async loadAllProductById(): Promise<void> {
        try {
            const ids = [];
            const importedProducts = await Promise.all(ids.map(async (id) => await this.forsageService.getProductById(id)));
            await this.productModel.insertMany(importedProducts);
        } catch (e) {
            return e?.message;
        }
    ...
    async loadAllProduct(): Promise<void> {
        ...
        try {
            // @ts-ignore
            const skus = await this.sku.paginate({ status: "NEW" }, { page: cronJob.times, limit: 1 });
            const { docs } = skus;
            const sku = docs[0];
            skuOption = sku.sku;
            ...
            if (_.isEqual(status, 200)) {
                ...
                if (success) {
                    ...
                    const savedNotFoundProduct = new this.notFoundModel({ sku: sku.sku, requestStatus: status });
            ...
            const savedNotFoundProduct = new this.notFoundModel({ sku: skuOption, requestStatus: 500 });
            const savedNotFound = await savedNotFoundProduct.save();
        } finally {
            cronJob.times += 1;
